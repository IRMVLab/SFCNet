...
class Trainer:
    def __init__(self):
        ...
        try:
            val_dataset = PCDataset('validation', cfg, info_list=cfg.info_list, profiler=self.profiler)
        except:
        ...
        class_weights = None
        if not cfg.noweightce:
            class_weights = torch.from_numpy(val_dataset.get_class_weight()).float().to(self.device).view(-1)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(tqdm_loader):
                ...
                _, end_points, _ = self.loss_func(end_points, self.val_dataset, self.criterion)
